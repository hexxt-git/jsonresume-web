...
${[
  ...
  ...(b?.profiles || []).map((p) =>
    p.url ? `<a href="${esc(p.url)}">${esc(stripUrl(p.url))}</a>` : '',
  ),
]
  .filter(Boolean)
  .map((s) => `<span>${s}</span>`)
...
${w.description ? `<div style="font-size:calc(12.5px * var(--fs-mult, 1));color:#777;font-style:italic">${md(w.description)}</div>` : ''}
...
${e.courses?.length ? `<p style="margin-top:4px;font-size:calc(12.5px * var(--fs-mult, 1))">Courses: ${e.courses.map((c) => esc(c)).join(', ')}</p>` : ''}
</div>`,
    )
    .join(''),
)}
...
${section(
  ...
  (resume.projects || [])
    .map(
      (p) => `<div class="entry">
<div class="entry-header"><h3>${link(p.url, p.name || '')}${p.entity ? `<span style="color:#555;font-size:calc(13px * var(--fs-mult, 1))"> - ${esc(p.entity)}</span>` : ''}${p.type ? `<span style="color:#777;font-size:calc(12.5px * var(--fs-mult, 1))"> (${esc(p.type)})</span>` : ''}</h3>${p.startDate ? `<span class="entry-meta">${dateRange(p.startDate, p.endDate)}</span>` : ''}</div>
${p.roles?.length ? `<div style="font-size:calc(12.5px * var(--fs-mult, 1));color:#555">Role: ${p.roles.map((r) => esc(r)).join(', ')}</div>` : ''}
...
${section('Technical Skills', resume.skills?.length ? `<dl class="skills-list">${(resume.skills || []).map((s) => `<dt>${esc(s.name)}${s.level ? ` <span style="font-weight:normal;color:#777;font-size:calc(0.833em * var(--fs-mult, 1))">- ${esc(s.level)}</span>` : ''}</dt><dd>${(s.keywords || []).map((k) => esc(k)).join(', ')}</dd>`).join('')}</dl>` : '')}
