...
.contact{text-align:center;color:#666;font-size:calc(13px * var(--fs-mult, 1));margin-bottom:10px}
...
.skill-item h3{font-size:calc(13px * var(--fs-mult, 1));display:inline}
...
${w.description ? `<div style="font-size:calc(12px * var(--fs-mult, 1));color:#777;font-style:italic">${md(w.description)}</div>` : ''}
...
${section(
  ...
  (resume.projects || [])
    .map(
      (p) => `<div class="entry">
<div class="entry-header"><h3>${link(p.url, p.name || '')}${p.entity ? `<span style="color:#777;font-size:calc(13px * var(--fs-mult, 1))"> - ${esc(p.entity)}</span>` : ''}${p.type ? `<span style="color:#888;font-size:calc(12px * var(--fs-mult, 1))"> (${esc(p.type)})</span>` : ''}</h3>${p.startDate ? `<span class="entry-meta">${dateRange(p.startDate, p.endDate)}</span>` : ''}</div>
${p.roles?.length ? `<div style="font-size:calc(12px * var(--fs-mult, 1));color:#777">Role: ${p.roles.map((r) => esc(r)).join(', ')}</div>` : ''}
${p.description ? `<p style="color:#444;margin-top:4px">${md(p.description)}</p>` : ''}
${p.highlights?.length ? `<ul>${p.highlights.map((h) => `<li>${md(h)}</li>`).join('')}</ul>` : ''}
${p.keywords?.length ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px">${p.keywords.map((k) => `<span style="background:#f5f5f5;padding:2px 8px;border-radius:3px;font-size:calc(11px * var(--fs-mult, 1));color:#555">${esc(k)}</span>`).join('')}</div>` : ''}
</div>`,
    )
    .join(''),
)}
...
${section('Skills', resume.skills?.length ? `<div class="skills-list">${(resume.skills || []).map((s) => `<div class="skill-item"><h3>${esc(s.name)}${s.level ? ` <span style="font-weight:normal;color:#888;font-size:calc(0.833em * var(--fs-mult, 1))">- ${esc(s.level)}</span>` : ''}: </h3><span>${(s.keywords || []).map((k) => esc(k)).join(', ')}</span></div>`).join('')}</div>` : '')}
...
${section('Volunteer Experience', (resume.volunteer || []).map((v) => `<div class="entry"><div class="entry-header"><h3>${esc(v.position)}, ${link(v.url, v.organization || '')}</h3><span class="entry-meta">${dateRange(v.startDate, v.endDate)}</span></div>${v.summary ? `<p style="color:#444;margin-top:4px">${md(v.summary)}</p>` : ''}${v.highlights?.length ? `<ul>${v.highlights.map((h) => `<li>${md(h)}</li>`).join('')}</ul>` : ''}</div>`).join(''))}
${section('Awards', (resume.awards || []).map((a) => `<div class="entry"><div class="entry-header"><h3>${esc(a.title)}</h3><span class="entry-meta">${esc(a.awarder)}${a.date ? `, ${a.date}` : ''}</span></div>${a.summary ? `<p style="color:#444;margin-top:4px">${md(a.summary)}</p>` : ''}</div>`).join(''))}
