...
${section(
  ...
  timelineEntries(
    (resume.work || []).map((w) => ({
      ...
      sub: `${link(w.url, w.name || '')}${w.location ? ` - ${esc(w.location)}` : ''}${w.description ? `<div style="font-size:calc(12px * var(--fs-mult, 1));color:#9ca3af;font-style:italic">${md(w.description)}</div>` : ''}`,
      detail: w.summary,
...
${section(
  ...
  timelineEntries(
    (resume.projects || []).map((p) => ({
      ...
      title: `${link(p.url, p.name || '')}${p.entity ? `<span style="color:#6b7280;font-size:calc(13px * var(--fs-mult, 1))"> - ${esc(p.entity)}</span>` : ''}${p.type ? `<span style="color:#9ca3af;font-size:calc(12px * var(--fs-mult, 1))"> (${esc(p.type)})</span>` : ''}`,
      sub: p.roles?.length
        ? `<div style="font-size:calc(12px * var(--fs-mult, 1));color:#9ca3af">Role: ${p.roles.map((r) => esc(r)).join(', ')}</div>`
        : undefined,
  ...
  timelineEntries(
    (resume.education || []).map((e) => ({
      ...
      extra: e.courses?.length
        ? `<div style="margin-top:4px;font-size:calc(12px * var(--fs-mult, 1));color:#9ca3af">Courses: ${e.courses.map((c) => esc(c)).join(', ')}</div>`
        : '',
    })),
  ),
)}
${section('Skills', resume.skills?.length ? `<div class="skills-grid">${(resume.skills || []).map((s) => `<div class="skill-group"><h3>${esc(s.name)}${s.level ? ` <span style="font-weight:normal;color:#9ca3af;font-size:calc(0.833em * var(--fs-mult, 1))">- ${esc(s.level)}</span>` : ''}</h3><div class="tags">${(s.keywords || []).map((k) => `<span class="tag">${esc(k)}</span>`).join('')}</div></div>`).join('')}</div>` : '')}
...
${section('Awards', (resume.awards || []).map((a) => `<div class="entry" style="margin-bottom:12px"><h3>${esc(a.title)}</h3><p style="color:#6b7280;font-size:calc(13px * var(--fs-mult, 1))">${esc(a.awarder)}${a.date ? ` - ${a.date}` : ''}</p>${a.summary ? `<p style="color:#6b7280;margin-top:4px">${md(a.summary)}</p>` : ''}</div>`).join(''))}
${section('Certificates', (resume.certificates || []).map((c) => `<div class="entry" style="margin-bottom:12px"><h3>${link(c.url, c.name || '')}</h3><p style="color:#6b7280;font-size:calc(13px * var(--fs-mult, 1))">${c.issuer || ''}${c.date ? ` - ${c.date}` : ''}</p></div>`).join(''))}
