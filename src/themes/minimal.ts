...
<div class="contact">${[b?.email, b?.phone, b?.location?.city ? `${b.location.city}${b.location.region ? ', ' + b.location.region : ''}` : '', ...(b?.profiles || []).map((p) => (p.url ? `<a href="${esc(p.url)}">${esc(p.network || p.username || '')}</a>` : ''))].filter(Boolean).join(' / ')}</div>
...
${section(
  ...
  (resume.projects || [])
    .map(
      (p) => `<div class="entry">
<div class="entry-row"><h3>${link(p.url, p.name || '')}</h3>${p.startDate ? `<span class="meta">${dateRange(p.startDate, p.endDate)}</span>` : ''}</div>
...
${section(
  ...
  (resume.education || [])
    .map(
      (e) => `<div class="entry">
<div class="entry-row"><h3>${esc(e.institution)}</h3><span class="meta">${dateRange(e.startDate, e.endDate)}</span></div>
${e.studyType || e.area ? `<div class="sub">${esc(e.studyType)}${e.area ? ` in ${esc(e.area)}` : ''}${e.score ? ` (${esc(e.score)})` : ''}</div>` : ''}
</div>`,
    )
    .join(''),
)}
${section('Skills', resume.skills?.length ? (resume.skills || []).map((s) => `<p class="skills-inline"><strong>${esc(s.name)}:</strong> ${(s.keywords || []).map((k) => esc(k)).join(', ')}</p>`).join('') : '')}
...
${section('Volunteer', (resume.volunteer || []).map((v) => `<div class="entry"><div class="entry-row"><h3>${esc(v.position)}, ${link(v.url, v.organization || '')}</h3><span class="meta">${dateRange(v.startDate, v.endDate)}</span></div>${v.highlights?.length ? `<ul>${v.highlights.map((h) => `<li>${esc(h)}</li>`).join('')}</ul>` : ''}</div>`).join(''))}
