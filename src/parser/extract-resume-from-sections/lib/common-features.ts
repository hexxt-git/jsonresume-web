...
export const hasOnlyLettersSpacesAmpersands = (item: TextItem) => /^[A-Za-z\s&]+$/.test(item.text);
...
const hasSeason = (item: TextItem) => SEASONS.some((season) => item.text.includes(season));
