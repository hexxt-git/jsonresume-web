...
// Name
export const matchOnlyLetterSpaceOrPeriod = (item: TextItem) => item.text.match(/^[a-zA-Z\s\.]+$/);

// Email
export const matchEmail = (item: TextItem) => item.text.match(/\S+@\S+\.\S+/);
const hasAt = (item: TextItem) => item.text.includes('@');

// Phone — international format first (longest match), then US format
...
// Location — "City, ST" (US) + "City, Country" (international)
